...
# v0.19: staged tightening inside each basin
E_OPEN = np.array(
    [
        [1.00, 0.95, 0.90],
        [0.95, 1.00, 0.92],
...
def softened_matrix(formal_matrix: np.ndarray) -> np.ndarray:
...
FIGURE_PATH = Path("figures/sim_v0_19.png")
SUMMARY_PATH = Path("notes/v0_19_summary.json")
...
def current_informal_matrix(gen: int, basin_idx: int) -> np.ndarray:
    return softened_matrix(current_formal_matrix(gen, basin_idx))
...
def save_summary_json(results: dict) -> None:
    payload = {
        "params": {
            ...
            "INFORMAL_MATRIX_RULE": "0.5 * (E_OPEN + current_formal_matrix)",
        },
...
def plot_results(results: dict, samples: list[dict]) -> None:
    ...
    ax.set_title("v0.19 Staged Eligibility-Graph Daisyworld: global entropy")
    ax.set_ylabel("H")

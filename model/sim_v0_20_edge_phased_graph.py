...
NUM_BASINS = 3
POP_SIZE_PER_BASIN = [800, 1000, 1200]
NUM_LINEAGES = 50
GENERATIONS = 100
TRIALS = 40
...
# Keep v0.19 behavior: no extra window fragility multiplier in graph branch.
...
# Eligibility graph classes
CLASS_PROBS = np.array([0.20, 0.40, 0.40], dtype=float)  # core, intermediate, periphery
...
# Edge-phased schedules over local phase p in [0,1]
# (start_phase, end_phase) for tightening from open -> tight.
EDGE_PHASES = {
    (0, 2): (0.0, 0.4),  # core-periphery earliest
    (2, 0): (0.0, 0.4),
    (1, 2): (0.2, 0.7),  # intermediate-periphery next
    (2, 1): (0.2, 0.7),
    (0, 1): (0.5, 1.0),  # core-intermediate latest bridge
    (1, 0): (0.5, 1.0),
    # within-class edges fixed for first pass
    (0, 0): None,
    (1, 1): None,
    (2, 2): None,
}
...
FIGURE_PATH = Path("figures/sim_v0_20.png")
SUMMARY_PATH = Path("notes/v0_20_summary.json")
...
def initial_counts_from_distribution(pop_size: int) -> np.ndarray:
    weights = np.random.lognormal(mean=0.0, sigma=INITIAL_HETEROGENEITY_SIGMA, size=NUM_LINEAGES)
    weights = normalize(weights)
    return np.random.multinomial(pop_size, weights)


def initial_classes() -> np.ndarray:
    return np.random.choice(np.arange(3), size=NUM_LINEAGES, p=CLASS_PROBS)
...
def local_phase(gen: int, basin_idx: int) -> float:
    start = WINDOW_STARTS[basin_idx]
    end = WINDOW_ENDS[basin_idx]
    if gen < start:
        return 0.0
    if gen >= end:
        return 1.0
    return float((gen - start) / max(1, end - start))
...
def interpolate_edge(open_val: float, tight_val: float, phase: float, schedule) -> float:
    if schedule is None:
        return open_val
    start, end = schedule
    if phase <= start:
        ...
        x = (phase - start) / (end - start)
    return float((1.0 - x) * open_val + x * tight_val)


def active_formal_matrix(gen: int, basin_idx: int) -> np.ndarray:
    phase = local_phase(gen, basin_idx)
    mat = E_OPEN.copy()
    for i in range(3):
        for j in range(3):
            sched = EDGE_PHASES[(i, j)]
            mat[i, j] = interpolate_edge(E_OPEN[i, j], E_TIGHT[i, j], phase, sched)
    # enforce symmetry against possible future schedule mismatch
    mat = 0.5 * (mat + mat.T)
    return mat


def active_informal_matrix(formal_matrix: np.ndarray) -> np.ndarray:
    return 0.5 * E_OPEN + 0.5 * formal_matrix


def class_weights(counts: np.ndarray, classes: np.ndarray) -> np.ndarray:
    totals = np.zeros(3, dtype=float)
    for c in range(3):
        totals[c] = counts[classes == c].sum()
    return normalize(totals)


def connectivity_scores(counts: np.ndarray, classes: np.ndarray, matrix: np.ndarray) -> np.ndarray:
    weights = class_weights(counts, classes)
    scores = np.ones(len(counts), dtype=float)
    for i in range(len(counts)):
        c = classes[i]
        scores[i] = float(np.dot(matrix[c], weights))
    return scores
...
def run_once() -> dict:
    basin_counts = np.stack([initial_counts_from_distribution(pop) for pop in POP_SIZE_PER_BASIN], axis=0)
    basin_classes = np.stack([initial_classes() for _ in range(NUM_BASINS)], axis=0)
    ...
    for gen in range(GENERATIONS):
        ...
        for b in range(NUM_BASINS):
            ...
            classes = basin_classes[b]
            skew = current_skew(gen, b)
            local_fragility_gain = current_fragility_gain(gen, b)
            formal_matrix = active_formal_matrix(gen, b)
            informal_matrix = active_informal_matrix(formal_matrix)
            ...
            formal_probs *= connectivity_scores(counts, classes, formal_matrix)
            ...
            informal_scores = connectivity_scores(counts, classes, informal_matrix)
            uniform_probs = np.ones(NUM_LINEAGES, dtype=float) / NUM_LINEAGES

            epp_probs = EPP_STATUS_WEIGHT * (attractiveness * informal_scores) + (1.0 - EPP_STATUS_WEIGHT) * uniform_probs
...
def run_experiments() -> dict:
    ...
    summary_stats = {key: aggregate_scalar([s[key] for s in summaries]) for key in summaries[0].keys()}
...
def save_summary_json(results: dict) -> None:
    payload = {
        "params": {
            ...
            "EDGE_PHASES": {str(k): v for k, v in EDGE_PHASES.items()},
            "INFORMAL_RULE": "0.5 * E_open + 0.5 * current_formal_matrix",
        },
...
def plot_results(results: dict, samples: list[dict]) -> None:
    ...
    ax.fill_between(gens, results["entropy_mean"] - results["entropy_std"], results["entropy_mean"] + results["entropy_std"], alpha=0.2)
    for s, e in zip(WINDOW_STARTS, WINDOW_ENDS):
        ax.axvspan(s, e, alpha=0.06)
    ax.set_title("v0.20 Edge-Phased Eligibility-Graph Daisyworld: global entropy")
    ax.set_ylabel("H")
    ...
    ax.fill_between(gens, results["active_mean"] - results["active_std"], results["active_mean"] + results["active_std"], alpha=0.2)
    for s, e in zip(WINDOW_STARTS, WINDOW_ENDS):
        ax.axvspan(s, e, alpha=0.06)
    ...
    for key in [
        ...
        val = ss[key]
        print(f"{key:24s} mean={val['mean']:.3f} std={val['std']:.3f} min={val['min']:.3f} max={val['max']:.3f}")
